...
: m_AdjacencyList()
, m_Stack() // TODO: depricate
, m_iCurrentRound(0)
, m_Queue()
////, m_InStack(adjacencyList.size()*2, false)
, m_Evaluated(adjacencyList.size()*2, -1)
{
    m_Stack.reserve(adjacencyList.size()*2); // TODO: depricate
...
void BiDoubleGraph::PushOnStack(int const vertex) // TODO(iamabel): depricate
{
    m_Stack.push_back(vertex);
    m_Evaluated[vertex] = m_iCurrentRound;
}

int BiDoubleGraph::PopOffStack()  // TODO(iamabel): depricate
{
...
////    m_Evaluated.clear(); m_Evaluated.resize(m_AdjacencyList.size(), false);
    ...
    for (size_t leftVertex = 0; leftVertex < numVerticesInLeftSide; ++leftVertex) {
        ...
        if (vMatching[leftVertex] != UNMATCHED_VERTEX) continue;
        PushOnStack(leftVertex);
    }
...
///////////////////////////// TODO(iamabel) //////////////////////////////////////////
void BiDoubleGraph::Enqueue(int const vertex)
{
    m_Queue.push(vertex);
    m_Evaluated[vertex] = m_iCurrentRound;
}

int BiDoubleGraph::Dequeue()
{
    int const front(m_Queue.front());
    m_Queue.pop();
    return front;
}
...
bool BiDoubleGraph::ComputeResidualPathBFS(vector<int> const &vMatching, vector<int> &vPath)
{
    ...
    // breadth first search, starting from imaginary start vertex
    // i.e., search starts from left-side.
    for (size_t leftVertex = 0; leftVertex < numVerticesInLeftSide; ++leftVertex) { //init the "left side"
        // only insert vertices without edges in matching, otherwise
        // imaginary first vertex has residual capacity 0 to that vertex.
        if (vMatching[leftVertex] != UNMATCHED_VERTEX) continue;
        Enqueue(leftVertex);
